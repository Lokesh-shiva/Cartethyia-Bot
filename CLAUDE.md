...
## TODO Next
1. Consider: `/echo-compare`, upgrade cost preview, DB-backed dismantle/merge alternative if dismantle-for-currency ever feels insufficient, SPD dodge/evasion mechanic (deferred — turn order shipped first).
2. Long-term/no rush: animated fight/boss visuals (currently only `/wish` has animation) — would need a real client-side animation approach (GIF sequence? external renderer?), not a small addition.

**Echo Skill**: `src/lib/echoSkills.ts` — 4th attack button granted by whichever echo sits in Main slot (slot 0), own cooldown (4 turns) separate from Resonance Skill. `bonuses.echoSkill` (setBonus.ts, resolved from `equippedSlot === 0`) exposes the `EchoSkillDef`. 21 bespoke 4-cost (boss) echo skills keyed by name in `BOSS_ECHO_SKILLS`; 1/3-cost echoes get a generic element-flavored `genericEchoSkill()`. `applyEchoSkill()` returns a result object combat loops apply generically (dmgMult/healHp/bonusEnergy/forceCrit/doubleHit/armsNextCrit/defShred/extraVibDrain/resetCdOnCrit/noDamage/setEnergyFull) — `NAMED_SET_TRIGGER` and `FLAT_LIFESTEAL` kinds are handled inline per combat loop since they need loop-local state (namedState mutations, shared lifesteal formula). Wired into all 6 combat loops (ascend/boss/dungeon/duel/raid/field-boss) plus `/encounter` chat fights (simpler — no namedState there, so `NAMED_SET_TRIGGER` just falls back to a plain hit). `nextAttackCritArmed`/DEF-shred are per-owner in solo fights, per-side in duel, and DEF-shred is raid-shared (`raid.bossDefShredTurnsLeft`) since raid has one shared boss target.
...
- `echoToBoss()` (combat.ts, used only by `/encounter`) had `vibBar` hardcoded to a flat 50 regardless of the enemy's gear-scaled HP — a single hit could drain it and shatter/one-shot instantly. Now `Math.max(40, hp * 0.5)`. `encounter.ts` also had a separate bug: player damage reduction used `enc.enemy.def` (raw unscaled) instead of `scaledEnemy.def` — defense scaling silently never applied to player-dealt damage.
- Field boss (4-cost) echo rarity: `scaledFieldBossRarityWeights()` in echoes.ts ramps 5★ odds from 20% (WL0) to 90% (WL8+) — WL-boss echoes (`/ascend`, `/boss`) already had their own escalating table and are untouched.
