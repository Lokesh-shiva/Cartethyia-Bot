...
function bannerEmbed(
  ...
  const targetBlock = target
    ? [
        ...
        ``,
        `\`BASE ATK\`  **${calcWishAtk(target,1)}** → **${calcWishAtk(target,90)}** at Lv 90`,
        `\`${target.subStatType.replace(/_/g," ")}\`  **+${calcWishSubStat(target.subStatBase,target.subStatScale,1)}%** → **+${calcWishSubStat(target.subStatBase,target.subStatScale,90)}%** at Lv 90`,
        `\`${target.hiddenSub1Type.replace(/_/g," ")}\`  *(unlocks Lv 20)*`,
        target.hiddenSub2Type ? `\`${target.hiddenSub2Type.replace(/_/g," ")}\`  *(unlocks Lv 50)*` : "",
        ``,
        `**Passive**`,
        `> ${target.passive}`,
        ``,
