...
function buildButtons(
  ...
  if (team?.isDevGuild && !team.isPlayerActiveNow && team.activeAllyCharacterId === "kaelith") {
    ...
    rows.push(new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId("boss_basic").setLabel("⚔️  Basic Attack").setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId("boss_skill").setLabel("🌪️  Windward Step").setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("boss_ultimate").setLabel("🌪️  Eye of the Squall")
        .setStyle(ButtonStyle.Success).setDisabled(team.concertoEnergy < 100),
      new ButtonBuilder().setCustomId("boss_flee").setLabel("🚪  Flee").setStyle(ButtonStyle.Danger),
    ));
...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    selCollector?.on("collect", async (sel: StringSelectMenuInteraction) => {
      ...
      let rhovenBossWeakenTurnsLeft = 0; // Rhoven's Ultimate — no generic "weaken the boss's own ATK" primitive here either, same shape as riloDefBuffTurnsLeft
      ...
      const runTurn = async () => {
        ...
        collector.on("collect", async (btn: ButtonInteraction | StringSelectMenuInteraction) => {
          ...
          if (btn.customId === "boss_basic") {
            ...
            const brenLingerMult = (isDevGuild && !isPlayerActive() && activeAllyCharacterId === "bren" && brenLingerTurnsLeft > 0) ? (1 + brenLingerBonus) : 1;
            ...
            } else if (isDevGuild && !isPlayerActive() && activeAllyCharacterId === "rhoven") {
              const forteBefore = solaceForte;
            ...
            } else if (isDevGuild && !isPlayerActive() && activeAllyCharacterId === "bren") {
              const forteBefore = solaceForte;
          ...
          if (btn.customId === "boss_skill" && isDevGuild && !isPlayerActive() && activeAllyCharacterId === "solace") {
            ...
            state.bossVibNow = Math.max(0, state.bossVibNow - Math.floor(playerDmg * result.vibFrac * totalVibMult));
          } else if (btn.customId === "boss_skill" && isDevGuild && !isPlayerActive() && activeAllyCharacterId === "rhoven" && allyKit) {
          ...
          } else if (btn.customId === "boss_skill" && isDevGuild && !isPlayerActive() && activeAllyCharacterId === "bren" && allyKit) {
          ...
          if (btn.customId === "boss_ultimate" && !(isDevGuild && !isPlayerActive())) {
          ...
          } else if (btn.customId === "boss_ultimate" && isDevGuild && !isPlayerActive() && activeAllyCharacterId === "rhoven" && allyKit) {
            const rhState = allyMechanicState as RhovenMechanicState;
            ...
            if (result.resetsConcertoEnergy) { concertoEnergy = 0; convergenceUsedThisTurn = true; }
          } else if (btn.customId === "boss_ultimate" && isDevGuild && !isPlayerActive() && activeAllyCharacterId === "bren" && allyKit) {
            const brState = allyMechanicState as BrenMechanicState;
            const result = allyKit.onUltimate(
          ...
          if (shatterTurnsLeft > 0) {
            ...
            // Bug fix (2026-09-18): these three effects all key off
            // activeBonuses/namedState — which dynamically follows whichever
            // unit (player or active ally) is currently fighting — but were
            // applying their heal/shield result to the hardcoded player body
            // regardless. When an ally with the matching element/named-set
            // was active, the heal/shield silently went to the WRONG (benched)
            // pool. Also guards against reviving a unit that just hit 0 from
            // the preceding lethal hit, before any KO check has run.
