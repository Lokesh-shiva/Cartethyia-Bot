...
function buildButtons(
  ...
  if (team?.isDevGuild && !team.isPlayerActiveNow && team.activeAllyCharacterId === "kaelith") {
    ...
    rows.push(new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId("fb_basic").setLabel("⚔️  Basic Attack").setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId("fb_skill").setLabel("❄️  Frostbind").setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("fb_ultimate").setLabel("❄️  Absolute Zero")
        .setStyle(ButtonStyle.Success).setDisabled(team.concertoEnergy < 100),
      new ButtonBuilder().setCustomId("fb_flee").setLabel("🚪  Flee").setStyle(ButtonStyle.Danger),
    ));
    ...
    selCollector?.on("collect", async (sel: StringSelectMenuInteraction) => {
      ...
      const cleanup = async (won: boolean) => {
        ...
        if (won) {
          ...
          let tempestShardsDropped = 0;
          if (fb.id === "tempest_ancient") {
            tempestShardsDropped = 1;
            await prisma.user.update({
              where: { id: interaction.user.id },
      ...
      const runTurn = async () => {
        ...
        collector.on("collect", async (btn: any) => {
          ...
          if (btn.customId === "fb_skill" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "solace") {
            ...
            state.bossVibNow = Math.max(0, state.bossVibNow - Math.floor(playerDmg * result.vibFrac * totalVibMult));
          } else if (btn.customId === "fb_skill" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "rhoven" && allyKit) {
          ...
          } else if (btn.customId === "fb_skill" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "feyra" && allyKit) {
          ...
          if (btn.customId === "fb_ultimate" && !(isDevGuild && !isPlayerActiveNow)) {
          ...
          } else if (btn.customId === "fb_ultimate" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "rhoven" && allyKit) {
            const rhState = allyMechanicState as RhovenMechanicState;
            ...
            if (allyConstellation >= 5) { brenLingerTurnsLeft = BREN_C5_LINGER_TURNS + 1; brenLingerBonus = BREN_C5_LINGER_BONUS; }
            if (result.resetsConcertoEnergy) { concertoEnergy = 0; }
          } else if (btn.customId === "fb_ultimate" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "feyra" && allyKit) {
            const feState = allyMechanicState as FeyraMechanicState;
            const result = allyKit.onUltimate(
            ...
            if (def.kind === "NAMED_SET_TRIGGER" && activeBonuses.activeNamedSetId === def.setId) {
              switch (def.setId) {
                ...
                case "FROSTVEIL_BASTION": {
                  ...
                  const instantVibDrain = Math.floor(fb.vibBar * 0.20);
          ...
          if (shatterTurnsLeft > 0) {
            ...
            let bossDmg   = Math.max(1, Math.floor(scaled.atk * move.damage * rhovenWeakenMult * feyraWeakenMult - activeDef * attunementDefMult * 0.4));
