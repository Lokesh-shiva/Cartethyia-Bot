...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const rawWeapon = await prisma.weapon.findFirst({
      where:  { userId: user.id, isEquipped: true },
      select: { name: true, weaponType: true, rarity: true, baseAtk: true, awakened: true, awakenedName: true, weaponBond: true },
    });

    const weapon: WeaponData | null = rawWeapon
      ? { name: rawWeapon.name, weaponType: rawWeapon.weaponType, rarity: rawWeapon.rarity, baseAtk: rawWeapon.baseAtk, awakened: rawWeapon.awakened, awakenedName: rawWeapon.awakenedName, weaponBond: rawWeapon.weaponBond }
      : null;
