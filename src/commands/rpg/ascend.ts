...
import { resolvePlayerBonuses, applyBonuses, apply4pcSkillBonus, apply4pcUltBonus, roll4pcDoubleHit, roll4pcBlock, apply5pcLowHpCrit, apply5pcFirstHit, apply5pcFullHpDmg, get5pcVibDrainMult, get5pcHpRegen, applyLifesteal, elemIgniteProc, elemFrostShield, elemDischargeEnergy, elemWindstrideMult, elemVoidSurgeHeal, elemRadianceRegen, elemRadianceCrit, applyAbilityAttack, abilityV2TurnRegen, effectiveSkillCooldown, hasQuickStrike, PlayerBonuses } from "../../lib/setBonus";
...
function buildButtons(
  ...
  if (team?.isDevGuild && !team.isPlayerActiveNow && team.activeAllyCharacterId === "kaelith") {
  ...
  } else if (team?.isDevGuild && !team.isPlayerActiveNow && team.activeAllyCharacterId === "rhoven") {
    rows.push(new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId("battle_basic").setLabel("⚔️  Basic Attack").setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId("battle_skill").setLabel("🌪️  Windward Step").setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("battle_ultimate").setLabel("🌪️  Eye of the Squall")
        .setStyle(ButtonStyle.Success).setDisabled(team.concertoEnergy < 100),
      new ButtonBuilder().setCustomId("battle_flee").setLabel("🚪  Flee").setStyle(ButtonStyle.Danger),
    ));
    ...
    rows.push(new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId("battle_basic").setLabel("⚔️  Basic Attack").setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId("battle_skill").setLabel("🩸  Bloodprice Cleave").setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("battle_ultimate").setLabel("🩸  Last Man Standing")
        .setStyle(ButtonStyle.Success).setDisabled(team.concertoEnergy < 100),
      new ButtonBuilder().setCustomId("battle_flee").setLabel("🚪  Flee").setStyle(ButtonStyle.Danger),
    ));
    ...
    rows.push(new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId("battle_basic").setLabel("⚔️  Basic Attack").setStyle(ButtonStyle.Primary),
      new ButtonBuilder().setCustomId("battle_skill").setLabel("❄️  Frostbind").setStyle(ButtonStyle.Secondary),
      new ButtonBuilder().setCustomId("battle_ultimate").setLabel("❄️  Absolute Zero")
        .setStyle(ButtonStyle.Success).setDisabled(team.concertoEnergy < 100),
      new ButtonBuilder().setCustomId("battle_flee").setLabel("🚪  Flee").setStyle(ButtonStyle.Danger),
    ));
...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    let rhovenBossWeakenTurnsLeft = 0; // Rhoven's Ultimate (Eye of the Squall) — no generic "weaken the boss's own ATK" primitive exists in this file (playerDebuffs/WEAKENED only ever applies TO the player elsewhere), so this is a dedicated per-fight counter, same shape as riloDefBuffTurnsLeft
    ...
    const runTurn = async () => {
      ...
      collector.on("collect", async (btn: MessageComponentInteraction) => {
        ...
        if (activeBonuses.activeNamedSetId === "RADIANT_CONVERGENCE" && btn.customId !== "battle_flee") {
        ...
        // Milestone 3.5b: whichever unit is currently acting/defending uses
        // ITS OWN resolved stats (isAllyActingOrDefending/activeBonuses
        ...
        const abilVibM = compositeVibMult(activeBonuses.abilityEffects);
        ...
        if (btn.customId === "battle_basic") {
          ...
          const base   = Math.max(1, Math.floor(activeAtk * teamMult * basicMoveMult * smolderMult * havocAtkMult * brenLingerMult * (1 - defReduction)));
          const extraElemBonus = glacioShieldTurnsLeft > 0 ? glacioShieldElemBonus : 0;
          let dmg      = Math.floor(base * (crit ? activeCritDmg : 1) * (isWeak ? 1.5 : 1) * (1 + activeBonuses.elemDmgBonus + extraElemBonus) * radiantDmgMult);
          ...
          state.playerEnergy = Math.min(100, state.playerEnergy + ENERGY_PER_TURN_ASCEND + elemDischargeEnergy(activeBonuses.elementPassive, crit) + ar_b.bonusEnergy + thunderboltEnergy);
          ...
          } else if (isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "rhoven") {
            const forteBefore = solaceForte;
          ...
          } else if (isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "feyra") {
            const forteBefore = solaceForte;
        ...
        if (btn.customId === "battle_skill" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "solace") {
          ...
          state.bossVibNow = Math.max(0, state.bossVibNow - Math.floor(playerDmg * result.vibFrac * totalVibMult));
        } else if (btn.customId === "battle_skill" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "rhoven" && allyKit) {
          const rhState = allyMechanicState as RhovenMechanicState;
          const forteEmpowered = isForteMaxed(solaceForte, RHOVEN_FORTE_CONFIG);
          ...
          const result = allyKit.onSkill(
            { playerHp: state.playerHp, playerHpMax: state.playerHpMax, allyHp, allyHpMax, turn: state.turn, isShattered: state.isShattered, mechanicState: rhState },
            { basicLevel: allyBasicLevel, skillLevel: allySkillLevel, ultimateLevel: allyUltimateLevel, introLevel: allyIntroLevel, forteLevel: allyForteLevel },
            allyConstellation,
          ...
          const crit = forteEmpowered || result.forceCrit || Math.random() < activeCritRate;
          abilCrit = crit;
        ...
        } else if (btn.customId === "battle_skill" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "bren" && allyKit) {
        ...
        } else if (btn.customId === "battle_skill" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "feyra" && allyKit) {
          ...
          let dmg      = Math.floor(base * (crit ? stats.critDmg : 1) * (isWeak ? 1.5 : 1) * (1 + activeBonuses.elemDmgBonus + extraElemBonusSkill) * radiantDmgMult);
          ...
          state.playerEnergy  = Math.min(100, state.playerEnergy + ENERGY_PER_TURN_ASCEND + elemDischargeEnergy(activeBonuses.elementPassive, crit) + ar_s.bonusEnergy);
        ...
        if (btn.customId === "battle_ultimate" && !(isDevGuild && !isPlayerActiveNow)) {
          ...
          let dmg      = Math.floor(base * (isWeak ? 1.5 : 1) * (1 + activeBonuses.elemDmgBonus + extraElemBonusUlt) * radiantDmgMult);
        ...
        } else if (btn.customId === "battle_ultimate" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "rhoven" && allyKit) {
          const rhState = allyMechanicState as RhovenMechanicState;
          ...
          // C4 also grants the active ally (Rhoven himself) a fresh Tempo stack.
          allyMechanicState = allyConstellation >= 4
            ? { ...(result.newMechanicState as RhovenMechanicState), tempoStacks: 1 }
            : result.newMechanicState;

          rhovenBossWeakenTurnsLeft = weaken.weakenTurns;
          rhovenBossWeakenPct = weaken.weakenPct;
          moveName += `\n◇ *${boss.name}* is left **WEAKENED** *(-${Math.round(weaken.weakenPct * 100)}% ATK, ${weaken.weakenTurns} turns)*`;

          if (result.resetsConcertoEnergy) { concertoEnergy = 0; convergenceUsedThisTurn = true; }
        } else if (btn.customId === "battle_ultimate" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "bren" && allyKit) {
          const brState = allyMechanicState as BrenMechanicState;
          const result = allyKit.onUltimate(
          ...
          if (allyConstellation >= 5) { brenLingerTurnsLeft = BREN_C5_LINGER_TURNS + 1; brenLingerBonus = BREN_C5_LINGER_BONUS; }
          if (result.resetsConcertoEnergy) { concertoEnergy = 0; convergenceUsedThisTurn = true; }
        } else if (btn.customId === "battle_ultimate" && isDevGuild && !isPlayerActiveNow && activeAllyCharacterId === "feyra" && allyKit) {
          const feState = allyMechanicState as FeyraMechanicState;
          const result = allyKit.onUltimate(
        ...
        if (btn.customId === "battle_echoskill" && activeBonuses.echoSkill) {
          ...
          if (def.kind === "NAMED_SET_TRIGGER" && activeBonuses.activeNamedSetId === def.setId) {
            switch (def.setId) {
              ...
              case "FROSTVEIL_BASTION": {
                ...
                const instantVibDrain = Math.floor(boss.vibBar * 0.20);
                state.bossVibNow = Math.max(0, state.bossVibNow - instantVibDrain);
          ...
          const energyGain = ENERGY_PER_TURN_ASCEND + elemDischargeEnergy(activeBonuses.elementPassive, crit) + result.bonusEnergy;
        ...
        if (state.bossHpNow <= 0) {
          await sendBattleCard(thread as any, { ...state, ...activeCardIdentity(), lastMove: `${moveName} — **BOSS DEFEATED!**` }, buildButtons(state, activeBonuses.echoSkill ? { name: activeBonuses.echoSkill.name, cooldown: echoSkillCooldown } : null, teamButtonContext()), teamStatusLine());
        ...
        if (shatterTurnsLeft > 0) {
          ...
          // Bug fix (2026-09-18): see raid.ts/boss.ts/field-boss.ts/dungeon.ts
          // — these three effects key off activeBonuses/namedState (dynamically
          // the active unit) but were applying the heal/shield to the
          // hardcoded player body.
