...
export async function generateUniqueAbilityV2(userId: string, persist = true): Promise<{
  ...
  const systemPrompt = [
    ...
    `- High crit DMG (>200%) + low/mid crit rate (<65%) → CRIT_RATE is the highest-value stat. Each +1% crit rate multiplies the existing crit DMG stack. Prioritise ON_CRIT triggers AND passive CRIT_RATE boosts.`,
    `- Crit rate truly capped (≥80%) → stop giving CRIT_RATE. Switch to CRIT_DMG or DMG_MULT to compound the existing rate.`,
    `- High crit rate (≥65%) + high crit DMG → prefer CRIT_DMG, ON_CRIT:DMG_MULT, STACK_DMG.`,
    `- Glass cannon (HP ≤6000) → BELOW_HP_PCT aggression or FIRST_ACTION burst to end fights fast.`,
