...
export async function generateUniqueAbilityV2(userId: string, persist = true): Promise<{
  ...
  const systemPrompt = [
    ...
    ``,
    `BUILD BIAS RULES (use these as a value hierarchy, not hard restrictions — the ability can still include any effect, just bias toward what amplifies the player most):`,
    `- High crit DMG (>200%) + crit rate below 65% → crit rate is the highest-value stat mathematically. Lean toward CRIT_RATE and ON_CRIT triggers as primary picks, but CRIT_DMG, ATK_MULT, and DMG_MULT are all still valid secondaries.`,
    `- Crit rate above 80% → crit rate has diminishing returns at that point; lean toward CRIT_DMG, DMG_MULT, or STACK_DMG instead, but CRIT_RATE is still fine as a secondary.`,
    `- Glass cannon (HP ≤6000) → lean toward BELOW_HP_PCT aggression or FIRST_ACTION burst; survival effects are also valid.`,
    `- Tanky / high HP → lean toward ABOVE_HP_PCT bonuses or sustained healing; offensive effects still work.`,
    `- Energy-stacking → lean toward GAIN_ENERGY and ON_ULT payoffs.`,
    `- Lifesteal-heavy → lean toward ON_HIT or BELOW_HP_PCT healing.`,
    `The goal is an ability that feels OPTIMAL for this build, not one that only fixes a single stat.`,
    ``,
