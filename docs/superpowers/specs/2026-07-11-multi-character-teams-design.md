...
- Real per-character equipment/loadout system (own weapon slot, own echoes, own gear independent of the player's account-level `/equip`). Milestone 2b hardcodes Wellspring directly onto Solace as a stopgap specifically because this doesn't exist yet — see the [Milestone 2b design spec](2026-07-11-milestone2b-wellspring-design.md) §1. Needed before Wellspring (or any signature weapon) can become a real, ownable, swappable item.
