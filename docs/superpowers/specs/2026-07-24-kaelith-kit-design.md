...
- **Standard-banner 4★ pool integration — a real code path, not a data change.** All character-pull machinery (`characterProgress.upsert`, dupe→token conversion, ownership checks, the 5★ reveal) currently lives *only* in `runCharacterBanner` (Solace's limited banner). `runStandardBanner`'s roll-result type is `{ tier: "4star"; weapon: WishWeapon }` — weapons and materials only, no character variant exists. Adding Kaelith therefore requires:
  - A new roll-result variant (`{ tier: "4star"; character: PlayableCharacterKit }`) alongside the existing weapon variant, with the 4★ roll picking from a combined pool of the 4 existing 4★ weapons **plus** any registered 4★ characters — matching how Genshin and WuWa both use a *mixed* character-and-weapon 4★ pool with a 10-pull guarantee.
  - Ownership check + `characterProgress.upsert` (first pull grants the character; dupes grant `constellationTokens`), generalized to take `characterId` instead of the hardcoded `"solace"` currently in `runCharacterBanner`.
  - Result-card rendering for a character result (portrait + name), distinct from the existing weapon result rendering.
  - **Reveal animation:** reuse the existing 4★ weapon reveal animation — no bespoke character animation for 4★s (that stays a 5★ privilege, matching Solace's dedicated `SOLACE_REVEAL_GIF`).
  - No rate or pity changes. Constellation token cost stays **1 per rank** (deliberately not raised: unlike Genshin's purchasable primogems, keys here are earned-only, so pull volume is already the real constraint).

- **Allies use their own element's passive.** `resolvePlayerBonuses(userId, characterId)` currently derives `elementPassive` from the **User row's** element regardless of `characterId`, so allies inherit whatever element the *player* picked. Allies should use their own element instead — Kaelith is Havoc, and Havoc's innate (+20% Lifesteal, VOID_SURGE on Shatter) is core to his relentless-attacker identity; inheriting a random player's element would undermine it entirely.

  **This changes Solace's live behavior and must be announced.** She becomes Spectro (+30% HP, +20% Elem DMG, RADIANCE regen) instead of inheriting the player's element — thematically correct for a healer, but existing owners will feel it as less damage and more survivability. Applying it to all characters rather than grandfathering Solace, so the rule stays consistent and doesn't need explaining forever.

- **`/guide` generalized, plus a per-character guide page.** The guide's Solace section (recruiting, leveling, gear, Starfall Shard source) becomes character-agnostic, with per-character detail driven off the kit registry — so Kaelith's entry (including his Umbral Shard source) and every future character's appear automatically rather than needing a hand-written section each time.
